...
def plot_map(shared):
    ...
    try:
        ...
        # Make a deep copy of the map and pose for plotting
        local_map = copy.deepcopy(shared.map)
        local_pose = copy.deepcopy(shared.pose)
    finally:
        shared.release()
        ...
        dx, dy = DX_DY_TABLE[local_pose[2]]
        ...
        x_start = local_pose[0] - dx / 2
        y_start = local_pose[1] - dy / 2
        ...
        for x_int, y_int in local_map.intersections:
            for h in range(8):
                dx, dy = DX_DY_TABLE[h]
                dx *= 0.5
                dy *= 0.5
                
            status = local_map.getintersection(x_int, y_int).streets[h]
            ...
            if shared.acquire():
                try:
                    ...
                    if command == 'explore':
                        ...
                        # Make a copy of the data we need before releasing the lock
                        if shared.map is None or shared.pose is None:
                            print("Cannot show map: Map or pose not initialized")
                            continue
                        local_map = copy.deepcopy(shared.map)
