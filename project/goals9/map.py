...
class Map:
    ...
    def find_nearest_unexplored(self, start_x, start_y):
        ...
        while queue:
            ...
            if (curr.x != start_x or curr.y != start_y) and self.has_unexplored_streets(curr.x, curr.y):
                # print(f"yo fix me plz, current x={curr.x}, start x = {start_x}, current y={curr.y}, start y={start_y}")
    ...
    def directed_explore_dijkstras(self, options):
        """
        Find the nearest intersection that is potentially closest to the goal given options
        that have elements (cost to goal, Intersection)
        
        Args:
            options (array): array of options organized (cost to goal, Intersection, heading) and is
                            not ordered
        ...
        for intersection in self.intersections.values():
            ...
            intersection.direction = None
            intersection.dijkstra_state = DIJKSTRA_STATE.UNVISITED
        # re-initialize our queue with our options
        queue = []
        for (cost, intersection, direction) in options:
            intersection.cost = cost
            intersection.direction = direction
            sortedInsert(queue, intersection)
